...
public:
    ...
    mapOptimization() : cuda_scan_to_map_opt(204800, 102400, 20480, 10240)
    {
    ...
    {   
        ...
        {
            // std::cout << "-------------------------------------------------- cloudInfo.odomAvailable == true ----------" << std::endl;

            Eigen::Affine3f transBack = pcl::getTransformation(cloudInfo.initialGuessX,    cloudInfo.initialGuessY,     cloudInfo.initialGuessZ, 
                                                               cloudInfo.initialGuessRoll, cloudInfo.initialGuessPitch, cloudInfo.initialGuessYaw);
            if (lastImuPreTransAvailable == false)
            {
                // std::cout << "-------------------------------------------------- lastImuPreTransAvailable == false ----------" << std::endl;

                lastImuPreTransformation = transBack;
                lastImuPreTransAvailable = true;
            } else {
                // std::cout << "-------------------------------------------------- lastImuPreTransAvailable == true ----------" << std::endl;
        ...
        {
            // std::cout << "-------------------------------------------------- cloudInfo.imuAvailable == true ----------" << std::endl;
    ...
    void scan2MapOptimizationWithCUDA()
    {
        ...
        static std::chrono::duration<double> total_dur_KNN_search;
        static std::chrono::duration<double> total_dur_one_frame;
        // float cur_dur_build_hashmap;
        // float cur_dur_KNN_search;
        float cur_dur_one_frame;
        float avg_dur_build_hashmap;
        float avg_dur_KNN_search;
        float avg_dur_one_frame;
        ...
        if (laserCloudCornerLastDSNum > edgeFeatureMinValidNum && laserCloudSurfLastDSNum > surfFeatureMinValidNum)
        {
            ...
            // cuda_plane_line_odometry
            surf_map_3d.resize(laserCloudSurfFromMapDS->size(), make_float4(0.0, 0.0, 0.0, 0.0));
            surf_pts_3d.resize(laserCloudSurfLastDS->size(), make_float4(0.0, 0.0, 0.0, 0.0));
            for(int i = 0; i < laserCloudSurfFromMapDS->size(); i++) {
                surf_map_3d[i] = { laserCloudSurfFromMapDS->points[i].x,
                                   laserCloudSurfFromMapDS->points[i].y,
                                   laserCloudSurfFromMapDS->points[i].z,
                                   laserCloudSurfFromMapDS->points[i].intensity };
            }
            for(int i = 0; i < laserCloudSurfLastDS->size(); i++) {
                surf_pts_3d[i] = { laserCloudSurfLastDS->points[i].x,
                                   laserCloudSurfLastDS->points[i].y,
                                   laserCloudSurfLastDS->points[i].z,
                                   laserCloudSurfLastDS->points[i].intensity };
            }

            corner_map_3d.resize(laserCloudCornerFromMapDS->size(), make_float4(0.0, 0.0, 0.0, 0.0));
            corner_pts_3d.resize(laserCloudCornerLastDS->size(), make_float4(0.0, 0.0, 0.0, 0.0));
            for(int i = 0; i < laserCloudCornerFromMapDS->size(); i++) {
                corner_map_3d[i] = { laserCloudCornerFromMapDS->points[i].x,
                                     laserCloudCornerFromMapDS->points[i].y,
                                     laserCloudCornerFromMapDS->points[i].z,
                                     laserCloudCornerFromMapDS->points[i].intensity };
            }
            for(int i = 0; i < laserCloudCornerLastDS->size(); i++) {
                corner_pts_3d[i] = { laserCloudCornerLastDS->points[i].x,
                                     laserCloudCornerLastDS->points[i].y,
                                     laserCloudCornerLastDS->points[i].z,
                                     laserCloudCornerLastDS->points[i].intensity };
            }

            Eigen::Matrix<float, 6, 1> trans6_init;
            trans6_init(0) = transformTobeMapped[0];    // roll
            trans6_init(1) = transformTobeMapped[1];    // pitch
            trans6_init(2) = transformTobeMapped[2];    // yaw
            trans6_init(3) = transformTobeMapped[3];    // x
            trans6_init(4) = transformTobeMapped[4];    // y
            trans6_init(5) = transformTobeMapped[5];    // z


            auto now_build_hashmap = std::chrono::system_clock::now();
            ...
            cuda_scan_to_map_opt.SetCornerPoints(corner_pts_3d);
            while( !(cuda_scan_to_map_opt.iter_count >= 30 || cuda_scan_to_map_opt.converged) ) {
                cuda_scan_to_map_opt.TransformSurfAndCornerPoints();
                ...
                cuda_scan_to_map_opt.CalcSurfAndCornerCoeff();
            ...
            // tf::Quaternion q; q.setRPY(transformTobeMapped[0], transformTobeMapped[1], transformTobeMapped[2]);
            // printf("q : %f , %f , %f , %f \n", q.w(), q.x(), q.y(), q.z());
    ...
    void publishOdometry()
    {
        ...
        //
        // std::ofstream traj_fstream(TRAJ_TXT_PATH, std::ios::app);
        // traj_fstream.setf(std::ios::scientific, std::ios::floatfield);
        // traj_fstream.precision(9);
        // static double timeStart = laserOdometryROS.header.stamp.toSec();
        // auto time_start =ros::Time().fromSec(timeStart) ;

        // traj_fstream << laserOdometryROS.header.stamp - time_start << " "
...
{
    // std::ofstream tmp_fstream(TRAJ_TXT_PATH, ios::out);
    // tmp_fstream.close();
